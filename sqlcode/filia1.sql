...
--------------------------------------------------------
  ...
  CREATE TABLE "BIBLIOTEKA"."KSIAZKI" 
   (	"ID_KSIAZKI" NUMBER(10,0), 
	...
	"ID_AUTORA" NUMBER(10,0), 
	"ID_FILII" NUMBER(2,0), 
	"STATUS" VARCHAR2(20 BYTE) DEFAULT 'dostêpna'
   ) SEGMENT CREATION IMMEDIATE 
  PCTFREE 10 PCTUSED 40 INITRANS 1 MAXTRANS 255 
 NOCOMPRESS LOGGING
  STORAGE(INITIAL 65536 NEXT 1048576 MINEXTENTS 1 MAXEXTENTS 2147483645
...
;
--------------------------------------------------------
--  DDL for Trigger TRG_REPL_AUTORZY_TO_F1
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE TRIGGER "BIBLIOTEKA"."TRG_REPL_AUTORZY_TO_F1" 
AFTER INSERT ON AUTORZY
FOR EACH ROW
BEGIN
    -- Sprawdzamy, czy ten autor pochodzi ST¥D (zak³adamy, ¿e Filia 2 ma ID od 5000 w górê)
    IF :NEW.ID_AUTORA >= 5000 THEN
        INSERT INTO AUTORZY@filia1_link (ID_AUTORA, IMIE, NAZWISKO)
        VALUES (:NEW.ID_AUTORA, :NEW.IMIE, :NEW.NAZWISKO);
    END IF;
END;

/
ALTER TRIGGER "BIBLIOTEKA"."TRG_REPL_AUTORZY_TO_F1" ENABLE;
--------------------------------------------------------
...
--------------------------------------------------------
  ...
  ALTER TABLE "BIBLIOTEKA"."KSIAZKI" ADD CONSTRAINT "CHK_KSIAZKI_STATUS_V2" CHECK (status IN ('dostêpna', 'wypo¿yczona', 'zarezerwowana')) ENABLE;
